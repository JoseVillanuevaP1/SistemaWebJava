...
public class InterGestionarVentas extends javax.swing.JInternalFrame {
    ...
    private void initComponents() {
        ...
        jPanel3.add(jLabel2, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 10, 100, -1));

        jLabel3.setFont(new java.awt.Font("Tahoma", 1, 14)); // NOI18N
        jLabel3.setHorizontalAlignment(javax.swing.SwingConstants.RIGHT);
        jLabel3.setText("Fecha:");
        jPanel3.add(jLabel3, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 40, 100, -1));
        ...
        jPanel3.add(jLabel5, new org.netbeans.lib.awtextra.AbsoluteConstraints(320, 40, 80, -1));

        txt_total_pagar.setFont(new java.awt.Font("Tahoma", 0, 12)); // NOI18N
        txt_total_pagar.setEnabled(false);
        jPanel3.add(txt_total_pagar, new org.netbeans.lib.awtextra.AbsoluteConstraints(130, 10, 170, -1));

        txt_fecha.setFont(new java.awt.Font("Tahoma", 0, 12)); // NOI18N
        txt_fecha.setEnabled(false);
        jPanel3.add(txt_fecha, new org.netbeans.lib.awtextra.AbsoluteConstraints(130, 40, 170, -1));
    ...
    private void jButton_actualizarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton_actualizarActionPerformed
        ...
        cliente = this.jComboBox_cliente.getSelectedItem().toString().trim();
        estado = this.jComboBox_estado.getSelectedItem().toString().trim();

        //obtener Id del cliente
        try {

            Connection cn = Conexion.conectar();
            PreparedStatement pst = cn.prepareStatement("select idCliente, concat(nombre,' ',apellido) as cliente from tb_cliente where concat(nombre,' ',apellido) = '"
                    + cliente + "'");
            ...
            System.out.println("Error al obtener Id del Cliente, en : " + e);

        }

        //actualizar Datos de Venta
        if (!cliente.equalsIgnoreCase("Selecciona Cliente")) {
            ...
            if (controlRegistrarVenta.actualizarVenta(cabeceraVenta, idVentas)) {
    ...
    private void limpiar() {
        ...
        this.jComboBox_cliente.setSelectedItem("Selecciona Cliente");
    ...
    private void CargarTablaVentas() {
        ...
        String sql = "select cv.idCabeceraVenta as id, concat(c.nombre,' ',c.apellido) as cliente, cv.valorPagar as total, cv.fechaVenta as fecha, cv.estado as estado "
        ...
        try {
            ...
            InterGestionarVentas.jTable_ventas = new JTable(mode1);
            InterGestionarVentas.jScrollPane1.setViewportView(InterGestionarVentas.jTable_ventas);

            mode1.addColumn("ID");
            mode1.addColumn("Cliente");
            mode1.addColumn("Total a Pagar");
            mode1.addColumn("Fecha Venta");
            ...
            System.out.println("Error al llenar tabla Ventas : " + e);
        }
        ...
        jTable_ventas.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                if (fila_point > -1) {
                    idVentas = (int) mode1.getValueAt(fila_point, columna_point);
                    EnviarDatosVentaSeleccionada(idVentas);
                }
            }
    ...
    private void EnviarDatosVentaSeleccionada(int idVentas) {
        try {
            Connection cn = Conexion.conectar();
            PreparedStatement pst = cn.prepareStatement("select cv.idCabeceraVenta, cv.idCliente, concat(c.nombre,' ',c.apellido) as cliente, cv.valorPagar, cv.fechaVenta, cv.estado "
                    + "from tb_cabecera_venta as cv, tb_cliente as c "
                    + "where idCabeceraVenta = '" + idVentas + "' and cv.idCliente = c.idCliente;");
            ResultSet rs = pst.executeQuery();
            if (rs.next()) {
                this.jComboBox_cliente.setSelectedItem(rs.getString("cliente"));
                ...
                if (estado == 1) {
                    ...
                    this.jComboBox_estado.setSelectedItem("Inactivo");
                }

            }
            cn.close();
        } catch (SQLException e) {
            System.out.println("Error al seleccionar Ventas: " + e);
        }
    }
